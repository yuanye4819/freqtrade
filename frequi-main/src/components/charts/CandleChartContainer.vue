  ...
  <div class="flex h-full">
    <div class="flex-fill w-full flex-col align-items-stretch flex h-full">
      <div class="flex me-0 items-center md:gap-2 gap-1 flex-wrap mx-1 md:mx-0">
        ...
        <div class="flex gap-1 md:gap-2 w-full md:w-auto">
          <BaseStringMultiSelectMenu
            ...
            :placeholder="$t('charts.selectPairsToPlot')"
            virtualize
            size="md"
          />
          <USelectMenu
        ...
        <BaseCheckbox v-model="settingsStore.multiPairSelection">
          <span class="text-nowrap">{{ $t('candleChart.multiPair') }}</span>
        </BaseCheckbox>
        <div class="ms-auto flex flex-wrap items-center gap-2">
          <BaseCheckbox v-model="settingsStore.showMarkArea">
            <span class="text-nowrap">{{ $t('candleChart.showChartAreas') }}</span>
          </BaseCheckbox>
          <BaseCheckbox v-model="settingsStore.useHeikinAshiCandles">
            <span class="text-nowrap">{{ $t('candleChart.heikinAshi') }}</span>
          </BaseCheckbox>
      ...
      <div v-else class="flex flex-col items-center justify-center h-full w-full">
        <span class="text-2xl font-semibold">{{ $t('candleChart.noPairSelected') }}</span>
      </div>
    </div>
    <DraggableModal
      v-model:open="showPlotConfigModal"
      :title="$t('candleChart.plotConfiguratorTitle')"
      class="max-w-xl"
      :description="$t('candleChart.plotConfiguratorDesc')"
      :overlay="false"
      :modal="false"
      :dismissible="false"
    >
