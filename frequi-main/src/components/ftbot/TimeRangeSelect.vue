...
<template>
  <div>
    <div class="flex gap-2">
      <UFormField :label="$t('timeRangeSelect.startDate')">
        <UInput id="dateFrom" v-model="dateFromText" :placeholder="$t('timeRangeSelect.datePlaceholder')" class="flex-1">
          <template #trailing>
            <UButton
              v-if="dateFromText"
              icon="mdi:close"
              ...
              variant="ghost"
              size="xs"
              :title="$t('timeRangeSelect.clearStartDate')"
              @click="dateFromText = ''"
            />
      ...
      <UFormField :label="$t('timeRangeSelect.endDate')">
        <UInput id="dateTo" v-model="dateToText" :placeholder="$t('timeRangeSelect.datePlaceholder')" class="flex-1">
          <template #trailing>
            <UButton
              v-if="dateToText"
              icon="mdi:close"
              ...
              variant="ghost"
              size="xs"
              :title="$t('timeRangeSelect.clearEndDate')"
              @click="dateToText = ''"
            />
    ...
    <div class="mt-1 text-start">
      {{ $t('timeRangeSelect.timerange') }}: <b>{{ timeRange }}</b>
    </div>
  </div>
</template>
