...
<template>
  <div>
    <div class="flex flex-wrap flex-row mb-2 justify-end items-center">
      <label class="text-xl ms-1 me-auto mb-0">{{ showBotOnly ? $t('botBalance.botBalance') : $t('botBalance.accountBalance') }} {{ $t('botBalance.balance') }}</label>
      <div class="flex flex-row gap-1">
        <UButton
          v-if="canUseBotBalance"
          color="neutral"
          :tooltip="!showBotOnly ? $t('botBalance.showingAccountBalance') : $t('botBalance.showingBotBalance')"
          :icon="showBotOnly ? 'mdi:robot' : 'mdi:bank'"
          @click="showBotOnly = !showBotOnly"
        />
        <UButton
          color="neutral"
          :tooltip="!hideSmallBalances ? $t('botBalance.hideSmallBalances') : $t('botBalance.showAllBalances')"
