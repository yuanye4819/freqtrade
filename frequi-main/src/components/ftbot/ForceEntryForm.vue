...
<template>
  <UModal
    ...
    :description="positionIncrease ? $t('trade.increaseExistingPosition') : $t('trade.manuallyEnterTrade')"
  >
    <template #body>
      <form ref="form" class="space-y-4" @submit.prevent="handleEntry">
        ...
        <UFormField :label="$t('trade.pair')" required>
          <UInput
        ...
        <UFormField :label="$t('trade.stakeAmountOptional', { currency: botStore.activeBot.stakeCurrency })">
          <UInputNumber
            v-model="stakeAmount"
            show-buttons
            :min="0"
