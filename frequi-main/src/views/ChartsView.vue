    ...
    <div v-if="botStore.activeBot.isWebserverMode" class="md:mx-3 mt-2 px-1">
      <UCard :ui="{ body: 'p-3 sm:p-3' }">
        <div class="flex items-center gap-2 mb-1">
          <span class="text-xl font-bold">{{ $t('charts.settings') }}</span>
          <InfoBox
            :hint="$t('charts.settingsHint')"
          />
        </div>
        <div
        ...
        >
          <UCollapsible v-model:open="exchange.customExchange">
            <div class="flex flex-row gap-5 items-center">
              <BaseCheckbox v-model="exchange.customExchange">{{ $t('charts.customExchange') }}</BaseCheckbox>
